...
{
    private static final Logger LOG = Logger.getLogger(GateResearcherAnnCollector_Deprecated.class.getName());
    ...
    //                                                          an.getEndNode().getOffset() + 100 < docGate.getContent().size() ? an.getEndNode().getOffset() + 100 :  an.getEndNode().getOffset()));
            ...
            LOG.info(String.format("%3d expressions in %s : ", n_expressions, docGate.getSourceUrl())); // + docXML.asXML()
            if(eOut == null) 
