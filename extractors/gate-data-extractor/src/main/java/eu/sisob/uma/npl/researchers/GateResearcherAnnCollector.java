...
public class GateResearcherAnnCollector extends AnnotatorCollector 
{
    private static final java.util.logging.Logger LOG = java.util.logging.Logger.getLogger(GateResearcherAnnCollector.class.getName());
    
    /**
     *
     * @param type
     */
    public GateResearcherAnnCollector(String type)
    {
        super(type);
    }
    ...
    //                                                          an.getEndNode().getOffset() + 100 < docGate.getContent().size() ? an.getEndNode().getOffset() + 100 :  an.getEndNode().getOffset()));
        ...
        LOG.info(String.format("%3d expressions in %s : ", n_expressions, docGate.getName())); // + docXML.asXML()
        if(eOut == null) 
            LOG.info("Output is null"); // + docXML.asXML()
        ...
        if(aoData.getVerbose())
        {
            ...
            try
            {
                ...
                LOG.log(Level.SEVERE, "Error writing verbose results. " + ex.toString());
            }
...
//      xmlDocument = xmlDocument.replace("</ProfessionalActivityCurrent>","</span>");      
      ...
      try 
      {
          ...
          LOG.log(Level.SEVERE, "The verbose file can not be created " + file_result.getPath(), ex);
      }
    }  
}
