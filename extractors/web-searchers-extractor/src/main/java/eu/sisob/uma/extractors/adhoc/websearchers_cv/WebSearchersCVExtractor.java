...
public class WebSearchersCVExtractor extends WebSearchersExtractorCommons {
    private static final Logger LOG = Logger.getLogger(WebSearchersCVExtractor.class.getName());
    ...
    static String cv_keywords_in_query = " AND (cv OR curriculum OR vitae OR cvitae)";
            ...
            try {
                while ((nextLine = reader.readNext()) != null) 
                {
                    ...
                    try{
                        ...
                        LOG.log(Level.SEVERE, "Error downloading " + url);
                    }
                }
            } catch (IOException ex) {
    ...
    public Document getDocumentFromPage(String url, int times, long wait_before, long wait_error){
        ...
        while(!yeah){
            try {
                ...
                doc = null;
                LOG.log(Level.SEVERE, "Error loading " + url + " " + ex.toString());
                try {                   
                    Thread.sleep(wait_error);
                } catch (InterruptedException ex1) {
                    LOG.log(Level.SEVERE, "Error sleeping: " + ex1.getMessage());
                }
                yeah = false;
                count++;
                if(count == times){
        ...
        if(doc != null && doc.select("div[class*=links_main] > a").size() > 0){
            ...
            {                    
                ...
                if(add){
                    ...
                    LOG.info("Select " + e.absUrl("href") + " - " + score + " - " + e.text());
                }                            
            }
