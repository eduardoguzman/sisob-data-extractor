...
public class InternalCVFilesExtractor {
    ...
    private static final java.util.logging.Logger LOG = java.util.logging.Logger.getLogger(InternalCVFilesExtractor.class.getName());   
    ...
    static String[] cv_keywords_in_name_list = new String[]{ "cv", "curriculum", "vitae", "cvitae", "vita", "curricula"};
...
//                                        }
                                ...
                                LOG.log(Level.SEVERE, "" + researcher_page_url + " could not loaded (out of memory)", ex2);
                        ...
                        if(content != null){
                            ...
                            for(Element link : links_worepeat ){
                                ...
                                if(b){
                                    LOG.info("CV found " + link.absUrl("href") + " (" + link.text() + ")");
                                ...
                                if(b){
                                    LOG.info("PUB found " + link.absUrl("href") + " (" + link.text() + ")");
                                    String href = link.absUrl("href");
...
//                 reader.close();  
    ...
    public static void download_files(File input_file, File results_dir, File zip_output_file, File output_file_2, StringWriter error_sw) {
        ...
        if(idResearcherWebAddress != -1 && idResearcherWebAddressType != -1 && idResearcherWebAddressExt != -1)
        {                
            LOG.info("Going to downloads results files");
...
//                        else
                        ...
                        while(!download_finish){
                            try{
                                ...
                                LOG.log(Level.SEVERE, "Error downloading " + url, ex);
                                num++;
                            }
                            if(max <= num)
                        ...
                        LOG.log(Level.SEVERE, "Error manage downloading " + url, ex);
                    }
                }
            } catch (Exception ex) {
                LOG.log(Level.SEVERE, "Error reading " + input_file.getName() + " " + ex.getMessage());
            }
            ...
            try {
                ...
                LOG.log(Level.SEVERE, "Error zipping results from " + input_file.getName());
            }
            
        }else{
            LOG.log(Level.SEVERE, "Headers incorrect " + input_file.getName());
        }
    }
