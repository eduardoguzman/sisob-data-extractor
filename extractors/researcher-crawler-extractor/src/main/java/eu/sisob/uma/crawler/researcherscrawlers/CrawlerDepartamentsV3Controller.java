...
{
    private static final Logger LOG = Logger.getLogger(CrawlerDepartamentsV3Controller.class.getName());
    ...
    public synchronized void addPossibleResults(CandidateTypeURL c)
    {
        if(!interesting_urls_detected.containsKey(c.sType))
        {
           ...
           //lst.add(c);
           if(CrawlerTrace.isTraceSearchActive()) LOG.info("\tCREATE NEW TYPE <" + c.sType + "> ");
        ...
        {            
            ...
            else if(c.sType.equals(SUBDEPARTMENTS_RESULT_TAG))
            {
                ...
                {
                    if(CrawlerTrace.isTraceSearchActive()) LOG.info("\tREFUSE BY NOT FROM DEPARTMENT <" + c.sType + "> " + c.toString());
                }
...
//                    }
        ...
        {
            LOG.log(Level.SEVERE, "Exception here => ", ex);
        } 
    }
