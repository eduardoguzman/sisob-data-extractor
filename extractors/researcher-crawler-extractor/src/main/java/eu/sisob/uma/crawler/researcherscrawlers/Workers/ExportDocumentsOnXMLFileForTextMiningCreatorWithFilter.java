...
{        
    private static final Logger LOG = Logger.getLogger(ExportDocumentsOnXMLFileForTextMiningCreatorWithFilter.class.getName());
...
//                                                org.jsoup.nodes.Document doc2 = Jsoup.connect(sURL).get();
                            ...
                            if(els.size() > 0)
                            {
                                ...
                                LOG.info("(" + xpathExp + ") => " + fileURL.getAbsolutePath());
                                hitsTable[0][0]++;
                        ...
                        {
                            LOG.log(Level.SEVERE, "ERROR URL: " + fileURL.getPath() + " Msg: " + ex.getMessage() + ".");
                        }
                    }
                }
            }
    ...
    protected void endActions() throws Exception
    {
        ...
        LOG.info("RESULTS: ");
        LOG.info("HITS: ");
        LOG.info("\t" + hitsTable[0][0] + "/" + (hitsTable[0][0] + hitsTable[1][0]));
        ...
        bw.write(sOut);
        bw.close();
        LOG.info("ResearcherPages.xml export!.");

        LOG.info(sContentForView);
    }
