 ...
 *  </infoblock>
 */
public class ExportDocumentsOnXMLFileForTextMiningCreator extends IteratorReseachersFile
{        
    private static final Logger LOG = Logger.getLogger(ExportDocumentsOnXMLFileForTextMiningCreator.class.getName());
     ...
     * @param destXmlFile
     */
    public ExportDocumentsOnXMLFileForTextMiningCreator(File source_file_xml, File downloadPagesDir, File dest_file_xml, LocalFormatType local_format_type)
    {
     ...
     * Note: Read comment in top of file.
        ...
        if(adirRW != null)
        {
            for ( File file : adirRW)
            {
                for ( Iterator i5 = elementResearcher.elementIterator("ResearcherWebAddress"); i5.hasNext(); )
                {
                    ...
                    {
                        LOG.log(Level.WARNING, "DIR NOT EQUAL: " + file.getPath() +  " != " + sAuxxx.replace("\\\\", "\\"));
                    }
                }
            }
        }                
