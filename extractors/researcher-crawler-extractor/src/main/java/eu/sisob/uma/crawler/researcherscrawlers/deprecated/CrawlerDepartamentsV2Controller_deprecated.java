...
{
   private static final Logger LOG = Logger.getLogger(CrawlerDepartamentsV2Controller_deprecated.class.getName());
   ...
   final boolean traceSearch = true;    
    ...
    public synchronized void addPossibleResults(CandidateTypeURL c)
    {
        if(!interesting_urls_detected.containsKey(c.sType))
        {
           ...
           //lst.add(c);
           if(traceSearch) LOG.info("\tCREATE NEW TYPE <" + c.sType + "> ");
        ...
        try        
        {
            if(c.sType.equals("Subdepartments"))
            {
                ...
                {
                    if(traceSearch) LOG.info("\tREFUSE BY NOT FROM DEPARTMENT <" + c.sType + "> " + c.toString());
                }
                else if(CandidateTypeURL.checkIfURLExistsInList(interesting_urls_detected.get(c.sType), c.sURL))
                {
                    if(traceSearch) LOG.info("\tREFUSE BY REPEAT <" + c.sType + "> " + c.toString());
                }
                else
                {
                    interesting_urls_detected.get(c.sType).add(c);
                    if(traceSearch) LOG.info("\tADD <" + c.sType + "> " + c.toString());
                }
            }
