...
{
    private static final java.util.logging.Logger LOG = java.util.logging.Logger.getLogger(CrawlerResearchesPagesV3Controller.class.getName());
    ...
    {          
        try
        {
            if(c.sType.equals(SUBSTAFFLIST_RESULT_TAG))
            {
                ...
                {
                    if(CrawlerTrace.isTraceSearchActive()) LOG.info("\tREFUSE BY NOT FROM STAFFLIST <" + c.sType + "> " + c.toString());
                }
                else if(CandidateTypeURL.checkIfURLExistsInList(interesting_urls_detected.get(c.sType), c.sURL))
                {
                    if(CrawlerTrace.isTraceSearchActive()) LOG.info("\tREFUSE BY REPEAT <" + c.sType + "> " + c.toString());
                }
                else
                {
                    interesting_urls_detected.get(c.sType).add(c);
                    if(CrawlerTrace.isTraceSearchActive()) LOG.info("\tADD <" + c.sType + "> " + c.toString());
            ...
            {     
                ...
                {
                    if(CrawlerTrace.isTraceSearchActive()) LOG.info("\tREFUSE BY NOT FROM STAFFLIST OR SUB <" + c.sType + "> " + c.toString());
                    ...
                    if(CrawlerTrace.isTraceSearchActive()) LOG.info("\tADD MAY BE <" + c.sType + "> " + c.toString());
        ...
        {
            LOG.log(Level.SEVERE, "Null pointer exception here => ", ex);
        }        
    }
