...
public abstract class TextMiningParserGate extends TextMiningParser {
    private static final Logger LOG = Logger.getLogger(TextMiningParserGate.class.getName());
    ...
    protected List getFilledCorpus(List<MiddleData> lstContent) throws Exception
    {
        ...
        try
        {
            ...
            for(MiddleData ao: lstContent)
            {
                ...
                {
                    LOG.log(Level.SEVERE, ex.getMessage() + " file: " + ((String)ao.getData_in()).replace("#", "%23"));
                    doc = null;
                }               
                ...
                {                    
                    ...
                    else if(doc.getContent().toString().length() > MAX_LENGTH_DOCUMENT)
                    {
                        ...
                        doc.setSourceUrl(url);
                        result.add(doc);
                        LOG.log(Level.WARNING, ((String)ao.getData_in()).replace("#", "%23") + " is empty."); 
                    }
                    else
                    {
                        result.add(doc);                
                ...
                {                    
                    ...
                    doc.setSourceUrl(url);
                    result.add(doc);
                    LOG.log(Level.WARNING, ((String)ao.getData_in()).replace("#", "%23") + " is too long.");                    
                }
            }
        }
        catch(Exception ex)
   ...
   /**
    ...
    {  
        LOG.info("Parsing with ANNIE  + extra JAPE + extra Gazetters (Running ANNIE)...");
