...
{
    private static final Logger LOG = Logger.getLogger(TextMiningParser.class.getName());
            ...
            while(mustRunning)
            {
                ...
                else
                {
                    if(!this.repOutput.isEmpty())
                    {
                        ...
                        LOG.info("Prepare for creating GATE's documents from repository)");
                        List<MiddleData> lst = this.getRepOutput().getData(literalIdParser, this.nInfoblocks);
                        LOG.info("Creating " + lst.size() + " GATE's documents from repository)");
                        ...
                        if(corpus.size() > 0)
                        {
                            ...
                            LOG.info("Parsing documents in: " + lTimeForParsing + " ms (" + (d / corpus.size()) + " ms each document)");
                            ...
                            LOG.info("Release corpus from memory");
    ...
    private void parseAnnotations(List corpus, List<MiddleData> lstOut)
    {
        ...
        LOG.info("Collecting annotation ...");
