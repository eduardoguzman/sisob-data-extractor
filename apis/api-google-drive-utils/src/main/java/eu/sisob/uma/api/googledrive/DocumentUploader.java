...
/**
 * A console application to demonstrate interaction with Google Docs API to
 * upload/update large media files using Resumable Upload protocol. 
 *
 */
public class DocumentUploader {
    private static final Logger LOG = Logger.getLogger(DocumentUploader.class.getName());
  ...
  private class FileUploadProgressListener implements ProgressListener {
    ...
    public synchronized void printResults() {
      ...
      LOG.info("Result: " + uploaded.size() + ", " + failed.size());
      if (uploaded.size() > 0) {
