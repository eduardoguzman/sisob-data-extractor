	...
	private int processPage(WebURL curURL) {
		...
		if (curURL == null) {
			...
			return -1;
                ...
                if(tracePageName) LOG.info("Process " + curURL.getURL());
		...
		if (statusCode != PageFetchStatus.OK) 
			...
			if (statusCode == PageFetchStatus.Moved) 
				...
				return PageFetchStatus.Moved;
                        ...
                        {
				LOG.log(Level.SEVERE, "Page was bigger than max allowed size: " + curURL.getURL());
			}
                        //ProjectLogger.LOGGER.info("Exit processPage");
			return statusCode;
		}
		...
		try 
                                ...
                                if (url != null) 
                                {
                                    ...
                                    if (newdocid > 0) 
                                    {
                                        if (newdocid != docid) 
                                        {
                                            ...
                                            if(traceLinkName) 
                                                LOG.info("Link " + webURL.toStringContent());
                ...
                {
                    e.printStackTrace();
                    LOG.log(Level.SEVERE, e.getMessage() + ", while processing: " + curURL.getURL());
		}
//                finally
//                {
//                    //ProjectLogger.LOGGER.info("Exit processPage");
