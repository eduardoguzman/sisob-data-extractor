	...
	public <T extends WebCrawler> void start(Class<T> _c, int numberOfCrawlers) {
		try {
			...
			while (true)
				...
				if (!someoneIsWorking)
                                {
                                        LOG.info("No one crawler seems working. Check if all are finished by condition.");
                                        ...
                                        if(bAllFinish)
                                        {
                                            LOG.info("All of the crawlers are finished by some condition. Finishing the process...");
					...
					LOG.info("It looks like no thread is working, waiting for 5 seconds to make sure...");
					sleep(5);                                        
					...
					if (!isAnyThreadWorking())
						...
						LOG.info("No thread is working and no more URLs are in queue waiting for another 5 seconds to make sure...");
						sleep(5);
        ...
        public void releaseResources()
        {
            ...
            if(docIDServer != null)
            {
                LOG.info("Releasing doc id server");
                try {
                   docIDServer.close();
                } catch(Exception ex) {
                    LOG.log(Level.SEVERE, ex.toString());
            ...
            if(env != null)
            {
                LOG.info("Releasing crawler controller env");
                try {
	...
	public void addSeed(String pageUrl) 
        {            
            ...
            if (canonicalUrl == null) {
                    LOG.log(Level.SEVERE,"Invalid seed URL: " + pageUrl);
                    return;
            }
            ...
            if (!RobotstxtServer.allows(webUrl, this.getDocIDServer())) {
                    LOG.info("Robots.txt does not allow this seed: " + pageUrl);
            } else {
